...
- **[[24_규칙도메인이전|log_24]] 바꿀 것** ("코치 힌트 없이 user가 먼저 패턴/구조 문제 꺼내기"): ✅ **적용됨**. Codex가 메서드명 안을 내밀었을 때 user가 먼저 "쿼리 두 번"이라는 구조 문제로 끌어올렸다. 코치가 가리키기 전에 user가 먼저 꺼낸 첫 번째 사례.
- **[[23_Slot값객체추출|log_23]] 바꿀 것** ("패턴 이름 연결"): **룰 자체가 폐기됨** (5번 참조). 사용자 자기 진단으로 "패턴 이름이 본인 학습에 가치 있는지 잘 모르겠다 — 정보탐색용으로만 의미 있다"고 판단. 외부 룰이 아니라 본인 호기심에 맡기는 것으로 변경.
...
~~"패턴 이름 연결" 룰~~ — 사용자가 "패턴 이름이 본인 학습에 가치 있는지 잘 모르겠다 — 정보탐색용으로만 의미 있다"고 진단. 코치가 외부 룰로 부과하던 것을 본인 호기심에 맡기는 형태로 이전. [[23_Slot값객체추출|log_23]] 이래 누적 3~4회 미룸 카운트도 같이 폐기.
