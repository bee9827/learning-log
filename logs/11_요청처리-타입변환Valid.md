...
- 효과적이었던 것과 그 이유
  ...
  [[01_Content-Type|log_01]]에서 DispatcherServlet 흐름을 배웠는데, 오늘 `ConversionService`와 `@Valid` 순서를 연결하면서 한 층 깊어졌다. 나선형 학습이 제대로 작동한 사례다.
...
4. **ConversionService 커스텀 타입 변환** — 직접 만든 타입(Name 등)을 PathVariable로 받으려면 어떻게 등록하는가
   → 종류: 코드 적용
5. **checked vs unchecked 예외와 롤백** — ([[04_JPA영속성-트랜잭션|log_04]]에서 이월)
   → 맥락: @Transactional이 걸린 메서드에서 checked 예외가 발생했을 때 롤백이 안 되는 시나리오
6. **트랜잭션 전파와 중첩 트랜잭션** — ([[04_JPA영속성-트랜잭션|log_04]]에서 이월)
