...
## 🔥 hot — arc 'step3 Rate Limit (호출량 상한)' (열림, log_49~). 토큰 버킷·인바운드/아웃바운드·429 백오프 재시도 구현 완료(PR #640). 남은 건 *튜닝·경계·대안 비교* — 아래 5개.

- [ ] **capacity↑(순간 버스트) vs refillPerSec↑(평균 처리량)** — 각각 뭘 키우나 / 종류: 흐름 파악 / 예고 log_49
- [ ] **인바운드 한도 ≠ 아웃바운드 한도** — 우리 처리용량 vs 상대가 허용한 몫, 같은 값 두면 안 되는 이유 / 흐름 파악 / log_49
- [ ] **fail-fast 거부 vs 토큰 찰 때까지 블로킹 대기** — 장단점(매끄러움 vs 스레드 점유·결정적 테스트 난이도) / 코드 적용 / log_49
- [ ] **read timeout 재시도("이미 처리됐을 수도") vs 429 재시도("아직 처리 안 됨")** — 멱등성과 연결한 안전장치 차이 / 흐름 파악 / log_49·44·#36
- [ ] **Rate Limit(양) vs 서킷 브레이커(연속 실패 차단)** — 차이·상호보완 / 흐름 파악 / log_49
