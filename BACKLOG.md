...
## 🔥 hot — 'gyoogle 컴퓨터 구조 순회' (하루 한 항목 학습, log_63에서 시작)

- [ ] **중앙처리장치(CPU) 작동 원리** — 자료 순서: `컴퓨터의 구성` 다음 / 종류: 흐름 파악 (명령어 인출·해독·실행과 레지스터의 역할)
- [ ] **캐시 메모리** — 자료 순서: CPU 작동 원리 다음 / 종류: 흐름 파악 (CPU↔RAM 속도 차이를 지역성으로 줄이는 구조, 오늘의 TLB와 비교)
