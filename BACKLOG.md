...
## 🔥 hot — arc '재시도 정책' (열림, 2026-07-07 선택). '능동적 실패/보상' arc의 미결 칸 + 한 가족 씨앗들(서킷 브레이커 코드 적용 log_54, 데드락 패자 재시도 log_56, 기존 429 백오프와 통합)을 한 실로.
...
- [ ] **서킷 브레이커 코드 적용** — 개념·3상태·트립 조건은 log_54에서 닫힘(재발명+Resilience4j 1:1 대조) / 종류: 코드 적용 (Resilience4j vs 직접 — 죽은 토스 시나리오에서 스레드 보호)
- [ ] **데드락 패자 재시도** — 예고 log_56(씨앗)·58 / 종류: 코드 적용 (CannotAcquireLockException = transient[오늘 축 그대로] → 앱 재시도 or 409 변환. 지금은 500)
