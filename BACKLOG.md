...
## 🔥 hot — 현재 arc: PENDING 생명주기 / 결제 "실패·미완료 처리" (log_37 적용 후 긴급)

> 코드 적용(log_37)으로 PENDING이 실제로 들어가자, 다각도 적대적 리뷰가 "미결제 방치·재시도·승격"을 구체 결함 8건으로 확인했다. 파킹했던 abandonment arc가 실물이 됐다 — 가까운 것부터 닫는다.

- [ ] **PENDING 만료 reaper** — 예고: log_37 / 종류: 코드 적용·설계 (미결제 PENDING이 슬롯 영구 점유[리뷰 high]. created_at TTL + @Scheduled, log_28 아웃박스 패턴 재사용)
- [ ] **트랜잭션 안 외부호출 분리 + Saga 보상** — 예고: log_31·37 / 종류: 코드 적용 (confirm()이 @Transactional 안에서 토스 호출 → 승인 후 후속단계 실패 시 돈↔상태 불일치[리뷰 high])
- [ ] **재시도 정책 — 몇 번·백오프·언제 포기** — 예고: log_36 / 종류: 코드 적용 (log_28 아웃박스 스케줄러 재시도)
- [ ] **대기 정합성 — PENDING 위 대기 금지 / 무료 승격 차단** — 예고: log_37 / 종류: 코드 적용 (findBySlotKeyForUpdate가 status를 무시 → 미결제 PENDING 위에 대기 가능[리뷰 high])
- [ ] **ArchUnit으로 ACL 경계 강제** — 예고: log_38 / 종류: 코드 적용 (토스 타입이 payment.toss 밖으로 못 나가게 테스트로 고정. "선이 지켜지나"를 자동 검증)

## 🧊 cold — 보관함 (클러스터별 / 평소엔 안 봄)

### 결제 — 단독 (abandonment arc는 hot으로 승격됨, log_37)
