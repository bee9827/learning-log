...
- [ ] **URL 입력 → 서버 도착까지의 네트워크 여정** — 예고: 종합_2부 / 종류: 흐름 파악. DNS로 IP 조회 → 라우터 → WAS 도착 → Tomcat이 하는 일. 애플리케이션 파이프라인(Spring 웹 흐름, log_15·21)보다 **한 층 아래(네트워크)**. WS/WAS 구분·서블릿 컨테이너 vs Servlet 객체·Tomcat이 HttpServletRequest 만들 때 하는 일까지. *학습자가 종합 2부 중 진짜 궁금해한 것 → 다음 세션 1번 타자.*
