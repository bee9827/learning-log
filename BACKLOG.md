...
## 🔥 hot — arc '동시성 손끝 증명 — lock-free(CAS) + 진짜 멀티스레드' (열림, 2026-07-03 선택). log_52의 synchronized(문 잠그기)에서 이어지는 실 — 같은 check-then-act 틈을 **락 없이** 닫는 CAS, 그리고 가드·상태 CAS가 진짜 경합에서 버티는지 증명까지.
...
- [ ] **동시성에서 정말 직렬화되나 — 진짜 멀티스레드 증명** — (cold '트랜잭션/모델링'에서 승격) 새치기 가드·상태 CAS 둘 다 H2+단일 트랜잭션으론 *메커니즘만* 증명 — 진짜 경합은 Testcontainers MySQL + 동시 스레드 필요 / 예고 log_28·47(§5) / 종류: 코드 적용
