...
## 🔥 hot — arc 'step3 Rate Limit (호출량 상한)' (열림, log_49~). 토큰 버킷·인바운드/아웃바운드·429 백오프 재시도 구현 완료(PR #640). **사이클 1(log_50)에서 #1·#2 닫음** — 남은 건 #3·#4·#5(거부정책·재시도안전성·서킷브레이커).
...
- [ ] **fail-fast 거부 vs 토큰 찰 때까지 블로킹 대기** — 장단점(매끄러움 vs 스레드 점유·결정적 테스트 난이도) / 코드 적용 / log_49·50
